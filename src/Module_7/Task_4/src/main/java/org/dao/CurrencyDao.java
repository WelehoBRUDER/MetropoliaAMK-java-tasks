...
public class CurrencyDao {
    ...
    public void persistTransaction(double amount, Currency source, Currency target) {
        EntityManager em = MariaDBJPAConnection.getInstance();
        em.getTransaction().begin();
        em.persist(new Transaction(amount, source, target));
        em.getTransaction().commit();
    }
